odoo.define('shopify_odoo_connector_sp.order_tbl', function (require) {
"use strict";
...
//
    ...
    /* FOR PIE CHART OF ORDERS */
    render_order_task:function(){
        var self = this;
        var w = 400;
        var h = 400;
        var r = h/2;
        var elem = this.$('.orders_graph');
        var colors = ['#70cac1', '#659d4e', '#208cc2', '#4d6cb1', '#584999', '#8e559e', '#cf3650', '#f65337', '#fe7139',
        '#ffa433', '#ffc25b', '#f8e54b'];
        var color = d3.scale.ordinal().range(colors);
        rpc.query({
            model: "shopify_odoo_connector_sp.instance_tbl",
            method: "get_orders_data",
        }).then(function (data) {
            console.log("--------------------------------------------------------")
            ...
            var segColor = {};
//            var vis = d3.select(elem[0]).append("div").attr("class", "row").append("div").attr("class", "col-md-8").append("svg:svg").data([data]).attr("width", w).attr("height", h).append("svg:g").attr("transform", "translate(" + r + "," + r + ")");
