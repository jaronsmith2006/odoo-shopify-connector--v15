...
class OrderClass(models.Model):
    ...
    instance_id = fields.Many2one("demo_shopify.instance_tbl", string="Instance", ondelete='cascade')
    ...
    def action_order_sync(self):
        for order in self:
            is_imported = self.env['demo_shopify.common'].sync_order_from_shopify(instance=self.instance_id, order=order.order_id)
    ...
    def action_order_print_invoice(self):
        return self.env.ref('demo_shopify.record_id').report_action(self)
