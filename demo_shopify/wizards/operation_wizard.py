...
class ImportOperations(models.TransientModel):
    ...
    instance = fields.Many2one("demo_shopify.instance_tbl", string="Instance", required=True, domain="[('state', '=', '1'),('active_status', '=', True)]")
    ...
    def import_data_from_odoo_to_shopify(self):
        if self.instance.id:
            ...
            if self.import_product:
                is_imported = self.env["demo_shopify.common"].import_product_from_shopify(self.instance)
            if self.import_orders:
                is_imported = self.env['demo_shopify.common'].import_orders_from_shopify(instance=self.instance)
            if self.import_customer:
                is_imported = self.env['demo_shopify.common'].import_customers_from_shopify(instance=self.instance)
    ...
    def export_product_by_action(self):
        if self.instance.id:
            is_exported = self.env["demo_shopify.common"].export_product_from_odoo_to_shopify_by_action(self.instance)
    ...
    def export_shopify_product_by_action(self):
        if self.instance.id:
            is_exported = self.env["demo_shopify.common"].export_shopify_product_by_action(self.instance)
...
class ExportOperations(models.TransientModel):
    ...
    instance = fields.Many2one("demo_shopify.instance_tbl", string="Instance", required=True, domain="[('state', '=', '1'),('active_status', '=', True)]")
    from_instance = fields.Many2one("demo_shopify.instance_tbl", string="Instance", required=True, domain="[('state', '=', '1'),('active_status', '=', True)]")

    # EXPORT PRODUCT
    def export_all_products(self):
        if self.instance.id:
            is_exported = self.env["demo_shopify.common"].export_all_products_from_shopify(to_instance=self.instance,
